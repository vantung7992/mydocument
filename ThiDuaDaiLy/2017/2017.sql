...
delete temp_2;
insert into temp_2(c4,n5,n15)
    select ma_dl,sum(decode(ma_ct,'SPM_GCN',so,0)),sum(decode(ma_ct,'SPM_DT',so,0)) from bh_dl_td_dt,temp_bc_dvi
        where ma_dvi=dvi and thang between round(b_ngayd/100,0) and round(b_ngayc/100,0)
        group by ma_dl;
...
delete temp_2 where exists(select sum(dt_tt) from bh_dl_td_sl where ma_dl=c4 and thang between 201701 and 201712
    having sum(dt_tt)<1000000000);
-- Tinh diem
update temp_2 a set n8=(select max(n7) from temp_2 b where a.c6=b.c6);
