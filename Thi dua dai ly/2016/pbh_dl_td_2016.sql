CREATE OR REPLACE PACKAGE PBH_DL_TD_2016 AS
-- Tong hop so lieu
...
CREATE OR REPLACE PACKAGE BODY PBH_DL_TD_2016 AS
...
BC_BH_THBT_MD_CD('','','','','','','',0,20141231,b_loi,'B');
if b_loi is not null then raise PROGRAM_ERROR; end if;
...
select a.ma_dvi,decode(nvl(d.ma_ct,' '),' ',d.ma,d.ma_ct) ma_dl,round(ngay_ht/100,0) thang,count(distinct a.so_id) so_kh,
	...
	where a.ma_dvi=b.ma_dvi and a.so_id=b.so_id and a.kieu_hd='G' and a.ttrang='D' and a.nv<>'NG'
	and a.ngay_ht between b_ngayd and b_ngayc
	and b.lh_nv not in ('050101','050102','050105','050201','050202','050203','070101','070201') and b.lh_nv not like '08%'
	and a.ma_dvi=b.ma_dvi and a.ma_kh=k.ma and k.loai not in ('08','11','13')
...
exception when others then rollback;
	if b_loi is null then b_loi:=substr('loi:'||SQLERRM||':loi',1,100); end if;
...
--|||||||||||    Tùng - Sua bao cao 2016    ||||||||||||||||||||||||||||||||||||
----Xoa BA0023900, 00604(Hai phong), Ninh Binh, Ha Nam
delete from kt_abic.temp_2 where c4 in ('BA0023900','00604','00213','00606');

----Ton tai 2 don vi Dong Da (00233 --> 08015)
update kt_abic.temp_2
  set (n8,n9,n10,n11,n12,n5,n7) = (select sum(n8),sum(n9),sum(n10),sum(n11),sum(n12),sum(n5),sum(n7) from kt_abic.temp_2 t where t.c4 in('08015','00233'))
  where c4 = '08015';
delete from kt_abic.temp_2 where c4='00233';
----Sam Son xac nhap Thanh Hoa (00902 --> 00901)
update kt_abic.temp_2
  set (n8,n9,n10,n11,n12,n5,n7) = (select sum(n8),sum(n9),sum(n10),sum(n11),sum(n12),sum(n5),sum(n7) from kt_abic.temp_2 t where t.c4 in ('00902','00901'))
  where c4 = '00901';
delete from kt_abic.temp_2 where c4='00902';
----Ton tai 2 don vi Ba ria Vung tau (00106 --> 00705)
update kt_abic.temp_2
  set (n8,n9,n10,n11,n12,n5,n7) = (select sum(n8),sum(n9),sum(n10),sum(n11),sum(n12),sum(n5),sum(n7) from kt_abic.temp_2 t where t.c4 in ('00106','00705'))
  where c4 = '00705';
delete from kt_abic.temp_2 where c4='00106';
--||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
----Ton tai 2 don vi Dong Da (00233 --> 08015)
update kt_abic.temp_2
  set (n5,n6) = (select sum(n5),sum(n6) from kt_abic.temp_2 t where t.c4 in('08015','00233'))
  where c4 = '08015';
delete from kt_abic.temp_2 where c4='00233';

----Sam Son xac nhap Thanh Hoa (00902 --> 00901)
update kt_abic.temp_2
  set (n5,n6) = (select sum(n5),sum(n6) from kt_abic.temp_2 t where t.c4 in ('00902','00901'))
  where c4 = '00901';
delete from kt_abic.temp_2 where c4='00902';

----Ton tai 2 don vi Ba ria Vung tau (00106 --> 00705)
update kt_abic.temp_2
  set (n5,n6) = (select sum(n5),sum(n6) from kt_abic.temp_2 t where t.c4 in ('00106','00705'))
  where c4 = '00705';
delete from kt_abic.temp_2 where c4='00106';
--||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
...
update temp_2 set c5=(select ma_nhno from bh_dl_nhno where ma_dl=c4 and nam=2016);
update temp_2 set c1=(select min(ma_dvi) from bh_dl_td_sl where thang between round(b_ngayd/100,0) and round(b_ngayc/100,0) and ma_dl=c4);
update temp_2 set n7=(select bt_cd from bh_dl_td_sl where ma_dvi=c1 and ma_dl=c4 and thang=round(b_ngayc/100,0));
update temp_2 set n6=n6+nvl(n7,0);
update temp_2 set (c6,c7)=(select ma_kv,kvuc from bh_dl_td_kvuc where ma_dvi=c1 and ma_dl=c4);

--Chia loai dai ly theo doanh thu thuc thu
...
--|||||||||||    Tùng - Bao cao 2016 |||||||||||||||||||||||||||||||||||||||||||
----Xoa BA0023900, 00604(Hai phong), Ninh Binh, Ha Nam
delete from kt_abic.temp_2 where c4 in ('BA002','00604','00213','00606');
----Ton tai 2 don vi Dong Da (00233 --> 08015)
update kt_abic.temp_2
  set (n5,n6) = (select sum(n5),sum(n6) from kt_abic.temp_2 t where t.c4 in('08015','00233'))
  where c4 = '08015';
...
-- Ty le tang truong
b_tl:=1.e9/365*(PKH_SO_CDT(b_ngayc)-PKH_SO_CDT(b_ngayd));
delete temp_2;
...
update temp_2 set n18=(n8-n3)/n3*100/3 where n3>b_tl; update temp_2 set n18=(n8-b_tl)/b_tl*100/3 where n3<=b_tl;
...
--Tung - Sua bao cao thi dua dai ly
delete from kt_abic.temp_2 where n15<20 or n16<20 or n17<20 or n8<5000000000;
----------

--Tinh diem tang truong ben vung
update temp_2 a set n19=(select max(n18) from temp_2 b where a.c6=b.c6);
update temp_2 set n20=round(n18/n19*50,2);
update temp_2 set n20=0 where n20<0;
...
procedure TH(b_ma_dvi varchar2,b_nsd varchar2,b_pas varchar2,b_dvi varchar2,b_ma_dl varchar2,
    b_ngayd number,b_ngayc number,cs1 out pht_type.cs_type)
AS
    b_loi varchar2(100); b_tl_m number; b_dt number; b_dt_m number; 
Begin
...
update temp_2 set c1=(select min(ma_dvi) from bh_dl_td_sl where thang between round(b_ngayd/100,0) and round(b_ngayc/100,0) and ma_dl=c4);
update temp_2 set n7=(select min(bt_cd) from bh_dl_td_sl where thang=round(b_ngayc/100,0) and ma_dvi=c1 and ma_dl=c4);
update temp_2 set n6=n6+n7;
update temp_2 set (c6,c7)=(select ma_kv,kvuc from bh_dl_td_kvuc where ma_dvi=c1 and ma_dl=c4);
...
--Tinh diem hieu qua
update temp_2 set n10=round(70-n9*100,2) where n9<1;
update temp_2 set n10=0 where n9>=1;
...
update temp_2 set c1=(select min(ma_dvi) from bh_dl_td_sl where thang between round(b_ngayd/100,0) and round(b_ngayc/100,0) and ma_dl=c4);
update temp_2 set c6=(select ma_kv from bh_dl_td_kvuc where ma_dvi=c1 and ma_dl=c4);

update temp_2 set (c5,n7)=(select ma_nhno,kh from bh_dl_nhno where ma_dl=c4 and nam=2016);
update temp_2 set n5=nvl(n5,0),n6=nvl(n6,0),n7=nvl(n7,0);
/*
-- Them phan chi khac
update temp_2 set n16=(select sum(ttoan_qd) from bh_cp where ma_dvi=c1 and FBH_DL_TDL(ma_dvi,ma_dly)=c4
	and ngay_ht between b_ngayd and b_ngayc and l_ct in ('CPDV_T','HTKT_T'));
update temp_2 set n6=n6+nvl(n16,0);
*/
-- Ty le thuc hien
update temp_2 set n8=round(decode(n7,0,0,n6/n7*100),2);
...
-- Ty le vuot ke hoach
b_dt:=(PKH_SO_CDT(b_ngayc)-PKH_SO_CDT(round(b_ngayd,-4)+101))/365;
update temp_2 set n9=round(decode(n7,0,0,n6/n7/b_dt*100),2)-100;
...
-- TONG HOP
update temp_6 set n1=nvl(n1,0),n2=nvl(n2,0),n3=nvl(n3,0),n4=nvl(n4,0),n5=nvl(n5,0),n6=nvl(n6,0);
delete temp_2;
insert into temp_2(c1,c6,c4,n1,n2,n3,n4,n5,n6,n7)
    select c1,c6,c4,sum(n1),sum(n2),sum(n3),sum(n4),sum(n5),sum(n6),sum(n1)+sum(n2)+sum(n3)+sum(n5)+sum(n6) from temp_6 group by c1,c6,c4;
...
open cs1 for select c1 ma_dvi,c11 ten_dvi,c4 ma_dl,c14 ten_dl,c5 ma_nh,c6 kvuc,c7 kvuc_ten,
    n1 diem_dt,n2 diem_hq,n3 diem_tt,n5 diem_dn,n6 diem_hsx,n8 diem_thuong,n7 tong_diem from temp_2
	where n7<>0
